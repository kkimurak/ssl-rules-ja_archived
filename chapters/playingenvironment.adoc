...
フィールドはすべての<<フィールドライン/Field Lines, フィールドライン>>から0.7mまで続いている。フィールドの外周には高さ0.1mのフェンスがあり、ロボットやボールがフィールド外に出ることを防ぐ。フィールドのさらに0.4m外側は<<主審/Referee, 主審>>と<<副審/Assistant Referee, 副審>>が通行するためのエリアである。 残りの0.3mはフィールドのマージンである。 +
