...
. If the ball is still <<Ball In And Out Of Play, in play>> after 1- seconds, the game is stopped and then continued by a <<Goal Kick, goal kick>> for the defending team.
...
試合が<<試合の再開/Resuming The Game, 再開>>されると、ボールは次の停止が発生するまではインプレイとみなされる。
